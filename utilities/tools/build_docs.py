...
Source of truth is one markdown file per topic under docs/manual/ (edit those
directly). This tool renders them to 40-column screen-code line records and packs
them into Magic Desk doc banks, which make_crt.py / build_disk.sh append after the
image banks and docs_pager.asm displays.
...
ROOT = Path(__file__).resolve().parent.parent   # the utilities/ dir
REPO = ROOT.parent                               # repo root (pristine mdbasic.asm)
ASM = REPO / "mdbasic.asm"
...
# VICE against the lowercase charset; the pager embeds these directly, no code change.
BOX_SCREEN = {
    "│": 0x5D,  # |  vertical
    "─": 0x40,  # -  horizontal
    "┌": 0x70,  # ,- top-left
