...
#   mdbasic.crt  - Magic Desk auto-start cartridge wrapping the PRG (with docs +
#                  utility banks when the markdown manual is present)
#
# The D64 is a copy of the template disk with mdbasic.prg replaced. The D81 is
# formatted fresh and mirrors every file from that D64. See make_crt.py for the
# cartridge format.
#
# Paths: mdbasic.asm and the template mdbasic.d64 come from the repo root (REPO);
# the utility sources, tools, docs and build dir live under utilities/ (ROOT).
#
...
REPO=$(cd "$ROOT/.." && pwd)             # repo root: pristine mdbasic.asm / .d64
cd "$ROOT"   # tmpx resolves .include paths against the cwd (src/, build/)
TEMPLATE="${MDBASIC_D64_TEMPLATE:-$REPO/mdbasic.d64}"
...
tmpx -l "$LST" -i "$REPO/mdbasic.asm" -o "$PRG"
...
# 4. Docs pager + utility banks: render the markdown manual (docs/manual/) and
#    assemble the menu tools into extra cart banks. Each tool assembles to $c000
#    (run address); strip the 2-byte load address to get the raw bank image.
DOCS_ARGS=""
if [ -d "$ROOT/docs/manual" ]; then
    ...
    tmpx -i "$ROOT/src/docs_pager.asm" -o "$OUTDIR/docs_pager.prg" >/dev/null
    tail -c +3 "$OUTDIR/docs_pager.prg" > "$PAGER"
    tmpx -i "$ROOT/src/menu.asm" -o "$OUTDIR/menu.prg" >/dev/null
    tail -c +3 "$OUTDIR/menu.prg" > "$HANDLER"
    tmpx -i "$ROOT/src/menu_body.asm" -o "$OUTDIR/menu_body.prg" >/dev/null
    tail -c +3 "$OUTDIR/menu_body.prg" > "$MENUBODY"
    tmpx -i "$ROOT/src/renum_tool.asm" -o "$OUTDIR/renum_tool.prg" >/dev/null
    tail -c +3 "$OUTDIR/renum_tool.prg" > "$RENUM"
    tmpx -i "$ROOT/src/move_tool.asm" -o "$OUTDIR/move_tool.prg" >/dev/null
    tail -c +3 "$OUTDIR/move_tool.prg" > "$MOVE"
    tmpx -i "$ROOT/src/copy_tool.asm" -o "$OUTDIR/copy_tool.prg" >/dev/null
    tail -c +3 "$OUTDIR/copy_tool.prg" > "$COPY"
    tmpx -i "$ROOT/src/convert_tool.asm" -o "$OUTDIR/convert_tool.prg" >/dev/null
    tail -c +3 "$OUTDIR/convert_tool.prg" > "$CONVERT"
    # package tool: boot stub first (pack_tool.asm embeds it via the generated
    # build/pack_stub.inc; make_crt.py patches its newvec/initclk sentinels from
    # the fresh listing)
    tmpx -i "$ROOT/src/pack_stub.asm" -o "$OUTDIR/pack_stub.prg" >/dev/null
    ...
    tmpx -i "$ROOT/src/pack_tool.asm" -o "$OUTDIR/pack_tool.prg" >/dev/null
    ...
    echo "Skipping docs: docs/manual not found"
fi
...
tmpx -i "$ROOT/src/boot.asm" -o "$OUTDIR/cart_boot.prg" >/dev/null
