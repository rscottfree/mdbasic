...
def asm(src: str, out: str, lst: str | None = None) -> bytes:
    ...
    if lst:
        cmd += ["-l", lst]
    cmd += ["-i", str(ROOT / "src" / src), "-o", out]
    # cwd=ROOT (utilities/): tmpx resolves .include paths against the cwd
    # (renum_tool.asm pulls src/edit_tool_common.asm, pack_tool.asm pulls
    # build/pack_stub.inc)
...
def build_cart() -> tuple[Path, int]:
    subprocess.run(["tmpx", "-l", "/tmp/mdbasic.lst",
                    "-i", str(REPO / "mdbasic.asm"),
                    "-o", "/tmp/mdbasic.prg"], check=True, capture_output=True)
    ...
    banks = make_crt.doc_banks(pager, idx, dat, handler,
                               ...
                               pack=pack, menu=menu)
    image = make_crt.image_from_prg(Path("/tmp/mdbasic.prg").read_bytes())
