...
GETBYTC= $b79b ;input an 8-bit unsigned int parameter with result in X reg
VAL    = $b7ad ;perform VAL
GETADR = $b7f7 ;convert FAC1 to unsigned 16-bit integer in LINNUM
...
outstr
 ...
 sta $fd
 beq gstr
 jsr GETSPA     ;alloc space in mem for string returning address ptr in FRESPC
...
setstrptr
 ...
 iny
 lda FRESPC     ;get lo byte of str ptr
 sta ($49),y    ;save it to variable's str ptr info
 iny
 lda FRESPC+1   ;get hi byte of str ptr
 sta ($49),y    ;save it to variable's str ptr info
donehere rts
;
filemax
...
getdskdev
 ...
 cpx #8         ;valid serial device numbers 8 to 12
 bcc illdev
 cpx #12
 bcs illdev
devdone
...
;save the byte
 ...
 sta (FRESPC),y ;store to variable
...
setval
 ...
 tay
 sta (FRESPC),y ;store directly to int value memory
valdone
 rts
setflt
 ...
 ldx FRESPC    ;copy the result in FAC1
 ldy FRESPC+1  ;to the variable memory
 jmp $bbd7     ;copy FAC1 to memory
setstr
 lda $fd       ;length of string read
 sta ($49),y   ;string length byte
 beq valdone   ;zero-length string
 jsr GETSPA    ;alloc new str return ptr in FRESPC and length in A reg
...
topause
 ...
 beq topause
chktimer
 lda LINNUM    ;zero flag indicates timeout reached
 ora LINNUM+1
endtimer rts
;
...
; SYS address [,a [,x [,y [,p]]]]
sys
 jsr getvaluez ;get 2-byte int into LINNUM
...
;
;*******************
; AUTO [ON]       :uses last used increment setting, default is 10
...
;this routine called during FIND when BASIC ROM is switched out
findlnr
...
restor
 beq oldrst
 jsr getline+3   ;find the line specified
...
clearerr
 ...
 tya
seterrln
 sta errline   ;make last error line 65535
 sty errline+1
 rts
raiseerr
...
close
 ...
 cmp #TOKEN_FILES
 beq clsfiles
gfn jsr GETBYTC+3 ;get file number
 txa
 beq nxtcls       ;ignore zero file number (invalid)
 pha              ;save file number
 jsr $e1cc        ;if open then close file and handle any error
...
msb sta MSIGX  ;x coord hibyte
 ...
 sta $fb
gety
 jsr chkcomm   ;stop now if no more params else skip over comma
...
onoff sta SPENA
;sprite color
scr jsr chkcomm ;stop now if no more params else skip over comma
 cmp #","       ;if another comma then omit param
 beq smcr
;get sprite color 0 to 16
 jsr GETBYTC+3  ;get 8-bit int param
 cpx #16        ;special color 16?
...
setspropt
 sta sprcolopt
;sprite multicolor flag
smcr jsr chkcomm ;stop now if no more params else skip over comma
 cmp #","        ;if another comma then omit param
 beq spntr
...
gtdata
 sty GARBFL
 jsr ckcom2    ;check for and skip over comma, misop err if missing
 jsr GETBYTC+3 ;get byte param value
 txa
 ldy GARBFL
 sta ($be),y
 iny
...
wave
 ...
 jsr ckcom2    ;check for and skip over comma, misop err if missing
 jsr GETBYTC+3 ;get waveform single byte operand value
 cpx #9        ;valid values 0 to 8
 bcs badwav
 tay           ;save char found after waveform param
 txa           ;convert waveform param value to a waveform bit pattern
 asl           ;move low nybble to high nybble
 asl
 asl
 asl
...
godraw
 ...
 txa
 ora LINNUM+1  ;is both hi and lo bytes 0
...
circle
 ...
 jsr ckcom2     ;check for and skip over comma, misop err if missing
 jsr GETBYTC+3  ;get x radius size
 txa
 beq illqty8    ;enforce valid range of 1 to 127
 bmi illqty8
 sta $be        ;save x-radius size to temp variable
 jsr ckcom2     ;check for and skip over comma, misop err if missing
 jsr GETBYTC+3  ;y radius size
 txa
 beq illqty8    ;enforce valid range of 1 to 127
 bmi illqty8
 sta $bf        ;y radius variable
 ...
 jsr GETBYTC+3  ;get options value
 stx $2a        ;circle options
circlept
...
norm2
 ...
 rts             ;KERNAL ROM ($e000-$ffff), BASIC ROM ($a000-$bfff)
;
...
text
 ...
 jsr ckcom2   ;check for and skip over comma, misop err if missing
 jsr getstr0  ;get text string param returning ptr in x and y regs
 stx LINNUM   ;in case the other params use a function
 sty LINNUM+1 ;that may need a temp str (VAL, INSTR etc.)
;set defaults for charset and x,y size
 lda #$d0     ;assume charset 0 at $d000
 sta INPFLG   ;charset hibyte temp var
 lda #1       ;default size value
 sta TRMPOS   ;temp var of x size
 sta VERCK    ;temp var of y size
 jsr comchk
 bne dotxt
 jsr getchrset+2
;sizes
 jsr comchk
 bne dotxt
 jsr getvalg  ;x size 1 to 31
 beq illqty8
 cmp #32
 bcs illqty8
 sta TRMPOS   ;save user specified x size
 jsr comchk
 bne dotxt
...
setvar
 ...
 sta (FRESPC),y  ;use buffer if string, variable memory for int and float (n/a)
...
;JOY(1) labeled "Port 1" uses CIA #1 Data Port B
...
joy1
 ...
 lda CIAPRA,y    ;data port A is joy 2 (port 2)
 and #%00011111  ;bit4=button state (0=pressed), bits0-3=direction
...
readpen
 ...
 lda LPENX-1,x   ;read 8-bit axis value of pen and
...
;finalize return value
 dex             ;x or y axis?
 bne pendone     ;y axis value is exact
...
;disable IRQs
 ...
 lda CIDDRA      ;get current data direction on data port A
 pha             ;save for restoring later
;select data direction
 stx CIDDRA      ;apply data direction on data port A
;select port 1 or 2
 lda #%10000000  ;bit 7 & 6 pattern: 10=A (port 2), 01=B (port 1)
 bcs setport     ;carry was set if pot num 3 or 4
 iny             ;adjust index for data port B (port 1)
 lsr             ;adjust bit pattern to select data port B (port 1)
setport
 sta CIAPRA      ;apply data port selection
;read buttons
...
;read the data
 lsr $62         ;even or odd pot num (1 to 4)?
...
;prepare return value
 ...
 bcs btnmsk      ;carry is set if odd pot num (1 or 3)
...
opget2
 ...
 jsr LINGET        ;convert asci decimal number to a 2-byte binary line number
linnul
 lda LINNUM        ;if no line num given
 ora LINNUM+1      ;then default to 65535
...
comma jsr CHRGET   ;advance TXTPTR
 beq missop
 rts
;*******************
...
getboolz beq missop
getbool
 jsr GETBYTC+3     ;get an unsigned single byte int parameter into x reg
 cpx #2            ;enforce value 0 or 1
 bcs illqty4
 txa               ;also return the result in the accumulator
 rts               ;and set processor Z and N flags
;*******************
getval15g jsr CHRGET
getval15z beq missop
getval15
 jsr GETBYTC+3     ;get an unsigned single byte int parameter into x reg
 cpx #16           ;enforce 0-15 range
 bcs illqty4
 txa               ;also return the result in the accumulator
 rts               ;with processor flags Z and N affected
;
missop
 ldx #31           ;missing operand error
 jmp (IERROR)      ;vector to print basic error message
illqty4
 jmp FCERR         ;illegal quanity error
;
;*******************
...
getfnparam
 ...
 rts                ;with processor flags Z and N affected
;*******************
;get an unsigned 2-byte int parameter (0-65535)
...
mcplot
 jsr GETBYTC+3   ;get color selection, multicolor selection index 1-3
 txa
 beq illqty4
 cmp #4          ;mc mode color index selection is 1,2 or 3
 bcs illqty4
;convert color pallete index to ptab offset 1=8, 2=16, 3=24
 asl
 asl
 asl
 sta mapcolbits  ;bit mask table offset for indexed color
 rts
;*******************
;get sprite number param (0-7) and calc its bit weight
sprnum
 jsr getvalz
...
less8 sta $be
 ...
 rts
;*******************
;determine hibyte for address of sprite pointers
ptrhi
...
;calc x2-x1
 ...
 sec
 sbc $fb   ;x1 lobyte
 sta $57   ;x distance lobyte
 lda $51   ;x2 hibyte
 sbc $fc   ;x1 hibyte
 sta $58   ;x distance hibyte
;calc y2-y1
 lda $52   ;y2
 sec
 sbc $fd   ;y1
...
;
txtright
 lda TRMPOS  ;char scale width (1-31)
 asl         ;scale * 8
 asl
 asl
 clc
...
txtprint
 jsr times8     ;multiply A reg value by 8 result in $be,$bf
 lda INPFLG     ;hibyte ptr to dot data of current charset
 clc
...
nextchar
 ...
 ldy #0
 lda (LINNUM),y ;get character to display on bitmap
...
;**************************
circel
;check multicolor mode, adjust if so
 lda SCROLX
 and #%00010000
 beq hirez
...
;
 ...
 sta $51
;
 lda $bf    ;y radius size
 asl        ;diameter = 2*radius
 sta LINNUM ;y diameter
 lda #0
 rol
...
wrdidx asl      ;convert note index to word index
 ...
 cmp #"-"       ;flat?
 beq fltshr
 and #%11110111 ;chars # and + are sharp
 cmp #"#"       ;sharp?
 bne regnote
...
dodraw
 jsr setdot
 dec LINNUM    ;decrement draw length
 bne dodraw2
 dec LINNUM+1
 sec           ;flag for done
...
;calc number of lines to fill
 ...
 dey
filines
 sty VERCK   ;fill up $ff or down $01
...
gethndshk
 ...
 ora LINNUM     ;handshake param value
 sta M51CDR
;
...
pdone rts
;set current i/o to output channel if needed
stdout
...
;make the MDBASIC temp file the current input device
startrd
 ldx mdbin      ;temp MDBASIC input file number
 jsr CHKIN      ;set device with file number as current input
 bcs pdone      ;stop now if error
 jsr CHRIN      ;skip 2 byte header
 jmp CHRIN      ;result in carry flag, set if error
;
